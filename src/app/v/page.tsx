...
async function getPosts(page: number, q?: string) {
  ...
  const where: Prisma.PostWhereInput = { published: true, showForFull: true };
  if (q?.trim()) {
    const k = q.trim();
    where.OR = [
...
async function getPickupPosts(): Promise<Post[]> {
  try {
    ...
    return await prisma.post.findMany({
      where: { published: true, isPickup: true, showForFull: true } as Prisma.PostWhereInput,
...
async function getRecommended(): Promise<Post[]> {
  try {
    return await prisma.post.findMany({
      where: { published: true, showForFull: true } as Prisma.PostWhereInput,
      orderBy: { views: "desc" },
      take: 5,
...
export default async function FullMemberPage({
  searchParams,
}: {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      <Header variant="full" homeHref="/v" />
      <main className="flex-1">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="lg:hidden pt-6 pb-4 border-b border-black/10">
            <form action="/v" method="get" className="relative max-w-2xl">
          ...
          <div className="lg:flex lg:gap-10">
                ...
                <section className="pt-8 pb-6 border-b border-black/10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {pickupPosts.map((post: Post) => (
                      <PostCard key={post.id} post={post} variant="pickup" basePath="/v" />
                    ))}
                  </div>
                </section>
              )}
              <section className="pt-8 pb-6">
                ...
                {data.posts.length > 0 ? (
                  <>
                    <div className="hidden md:block">
                      {data.posts.map((post: Post) => (
                        <PostCard key={post.id} post={post} variant="list" basePath="/v" />
                      ))}
                    </div>
                    <div className="md:hidden grid grid-cols-2 gap-x-4 gap-y-6 pt-6">
                      {data.posts.map((post: Post) => (
                        <PostCard key={post.id} post={post} variant="grid" basePath="/v" />
                      ))}
                    </div>
                    <Pagination currentPage={data.page} totalPages={data.totalPages} basePath="/v" />
                  </>
                ) : (
                  <div className="text-center py-20">
                    ...
                    <p className="text-black/40 text-sm mt-1">{q ? "検索条件に一致する記事がありません" : "正会員向けの記事はありません"}</p>
                  </div>
                )}
              </section>
            </div>
            <aside className="hidden lg:block w-[320px] shrink-0 pt-10">
              <div className="mb-6">
                <form action="/v" method="get" className="relative">
                  <input type="hidden" name="page" value="1" />
                  ...
                  <input type="search" name="q" defaultValue={q} placeholder="記事を検索" className="w-full pl-10 pr-3 py-2.5 text-sm bg-black/5 border border-black/10 rounded-lg text-black placeholder:text-black/40 focus:outline-none focus:ring-2 focus:ring-black/10 focus:border-black/20" aria-label="記事を検索" />
                  <button type="submit" className="absolute right-1.5 top-1/2 -translate-y-1/2 px-2 py-1 text-xs font-medium text-black/60 hover:text-black">検索</button>
                </form>
              </div>
                  ...
                  <ul className="space-y-4">
                    {recommended.map((post) => (
                      <li key={post.id} className="border-b border-black/5 last:border-0 pb-4 last:pb-0">
                        <Link href={`/v/${post.slug}`} className="group flex gap-3">
                          <div className="w-28 h-20 shrink-0 rounded-sm overflow-hidden bg-black/5">
                            {post.eyecatch ? (
                              ...
                              <div className="w-full h-full flex items-center justify-center"><span className="font-black text-lg text-black/20">iPS</span></div>
                            )}
                          </div>
                          <div className="flex-1 min-w-0">
                    ...
                    {banners.map((b) => (
                      <li key={b.id}>
                        <a href={b.url} target="_blank" rel="noopener noreferrer" className="block border border-black/10 rounded-sm overflow-hidden hover:opacity-90 transition-opacity">
                          ...
                          <p className={`font-medium text-sm text-black text-center ${b.imageUrl ? "py-2" : "py-3 px-3"} bg-white`}>{b.label}</p>
                        </a>
                      </li>
                    ))}
                  </ul>
