...
async function getPosts(page: number, q?: string) {
  ...
  const where: Prisma.PostWhereInput = { published: true, showForGeneral: true };
  if (q?.trim()) {
    const k = q.trim();
    where.OR = [
...
async function getPickupPosts(): Promise<Post[]> {
  try {
    ...
    return await prisma.post.findMany({
      where: { published: true, isPickup: true, showForGeneral: true } as Prisma.PostWhereInput,
...
async function getRecommended(): Promise<Post[]> {
  try {
    return await prisma.post.findMany({
      where: { published: true, showForGeneral: true } as Prisma.PostWhereInput,
      orderBy: { views: "desc" },
      take: 5,
...
export default async function GeneralMemberPage({
  searchParams,
}: {
      ...
      <main className="flex-1">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="lg:hidden pt-6 pb-4 border-b border-black/10">
            <form action="/g" method="get" className="relative max-w-2xl">
          ...
          <div className="lg:flex lg:gap-10">
              ...
              <section className="pt-8 pb-6">
                ...
                {data.posts.length > 0 ? (
                  ...
                  <div className="text-center py-20">
                    ...
                    <p className="text-black/40 text-sm mt-1">{q ? "検索条件に一致する記事がありません" : "一般会員向けの記事はありません"}</p>
                  </div>
                )}
              </section>
            </div>
            <aside className="hidden lg:block w-[320px] shrink-0 pt-10">
              <div className="mb-6">
                <form action="/g" method="get" className="relative">
                  <input type="hidden" name="page" value="1" />
